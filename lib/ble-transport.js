...
export class BleTransport {
  ...
  async connect() {
    ...
    const candidates = [
      { uuid: NB_SERVICE, write: NB_WRITE, notify: NB_NOTIFY },
      { uuid: NORDIC_SERVICE, write: NORDIC_WRITE, notify: NORDIC_NOTIFY },
    ];
    let chosen = null;
    for (const c of candidates) {
      ...
      for (const ch of chars) {
        ...
        this.onLog('GATT', `${service.uuid.slice(4, 8)} / ${ch.uuid}: ${props.join(', ')}`);
      }
      if (chosen) continue;
      chosen = { service, write: c.write, notify: c.notify, chars };
    }
    if (!chosen) throw new Error('Auf dem Gerät wurde kein Ninebot-Bluetooth-Dienst gefunden. Ist das der Roller?');

    this.serviceUuid = chosen.service.uuid;
    this.writeChar = await chosen.service.getCharacteristic(chosen.write);
    this.notifyChar = await chosen.service.getCharacteristic(chosen.notify);

    // Listen on every notifying characteristic of the service: firmware that
    // was modified may answer on a different channel than the official one.
    for (const ch of chosen.chars) {
      ...
      ch.addEventListener('characteristicvaluechanged', (ev) => {
        ...
        this.rawCount += 1;
        this.onLog(`RAW ${ch.uuid.slice(4, 8)}`, hexBytes(bytes));
