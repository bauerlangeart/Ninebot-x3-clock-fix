...
export class NbSession {
  ...
  async handshake(btName, {
    ...
    firstSnCounter = 3, setPwdRepeatMs = 500,
  } = {}) {
    let pre = null;
    let sawReject = false;
    ...
    // The documented E125S flow starts at counter 2. A capture of SHU pairing
    // a Max G3 shows its first SN frame with counter 3, so we do the same.
    this.crypto.startSn(firstSnCounter - 1);
